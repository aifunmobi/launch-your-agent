...
The agent you build is **local**: a Claude Code **subagent** (`.claude/agents/<name>.md`), a **slash command** (`.claude/commands/<name>.md`) that runs it, a **local grader** subagent that scores the output, and a `settings.json` that gates its tools and networking. It runs **on the founder's own machine, inside the Claude Code they're already signed in to** — **no separate Anthropic API key, nothing billed per run** (one caveat on the headless/scheduled path — see the "No Anthropic API key" invariant in Ground rules). The only prerequisite is the thing they used to start this skill: Claude Code, installed and signed in. Scheduling is local (launchd/cron, or Claude Code's `/schedule`), not a cloud deployment.
...
- **No Anthropic API key — ever, and the agent enforces it.** This fork runs entirely on the founder's signed-in Claude Code session. **Never** instruct the founder to create, fetch, paste, export, or install an `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` (or to enable a Bedrock/Vertex backend), and never write one into `.env`, `settings.json`, `environment.md`, or the shell environment — `.env` is for *third-party connector* tokens only. If any path you're about to take would require an Anthropic key, **stop and say so**: this fork is key-free by design (the upstream cloud repo is the one that needs a key). This matters because the headless/scheduled path (`claude -p`) silently bills as **API usage** if a key/token is in the environment, and Anthropic has a paused-for-now plan to bill that path separately (anthropics/claude-code #43333, #37686). Enforcement is real, not just prose: **every generated agent includes `no-api-key-guard.sh`** (copied verbatim from `references/no-api-key-guard.sh`), and `run.sh` / `evals/run-evals.sh` / any scheduled job **source it before the first `claude` call** so an unattended run *stops with a message* instead of billing as API if a key is ever present. Keep the public claim qualified — "no *separate* API key, nothing billed per run" + the headless/scheduled caveat — not an unconditional "nothing is ever billed".
...
my-agent/
  ...
  no-api-key-guard.sh       # 🔐 sourced by every unattended runner — stops the run if an Anthropic key/token is present
...
- **📖 What can this do?** → four lines, no wall of text: (1) a local agent is a Claude Code subagent + a `/<name>` command + a local grader + (optionally) a schedule, all in a portable folder; (2) it runs on the Claude Code login they already have — no separate API key, nothing billed per run; (3) 2–3 example outcomes from the examples bank; (4) then re-show this menu.
