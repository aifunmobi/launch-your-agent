...
5. **Hygiene sweep — quiet by default.** Do the sweep and **fix silently what you can**: no literal dates in the scheduled task (the kickoff/`first_prompt.txt` should say "today" / "the last 7 days", never a hard-coded date that goes stale); if a third-party connector is wired, `.env` is `chmod 600` and listed in `.gitignore`; **the key-free guard is in place** — `no-api-key-guard.sh` exists, is `chmod +x`, and `run.sh` (plus `evals/run-evals.sh` and any scheduled job) sources it before the first `claude` call (restore it from `launch-your-agent/references/no-api-key-guard.sh` if missing); and if there's no golden eval case yet, save a passed run's output as `evals/case-01/expected.md`. Only mention something if it's materially relevant — i.e. the founder must act (a secret that touched chat → rotate it; an Anthropic key/token found in the env or `.env` → remove it, since headless runs would bill as API; a literal date you can't patch without their say-so) or it changes how they use the agent. No "✅ all good" lists.
6. **Last words** — one short line: everything lives in this portable `my-agent/` folder — drop it on any machine with `claude` and re-run it via `LAUNCH.md`; there's no Console and nothing billed per run (the guard keeps unattended runs on your subscription); and they can rerun `/wrap-up` whenever they want a fresh picture.
